package com.plcoding.tracker_presentation.search.original
...
class SearchViewModel @Inject constructor(
            ...
            is SearchEvent.OnAmountForFoodChange -> {
                state = state.copy(
                    trackableFood = state.trackableFood.map {
                        // オブジェクトの比較やけど、これってどうやって先に入ってるやつと比較するんや?
                        // 検索した結果をリストにして詰めるっていうのはわかるけど、それをどうmapで比較するん?
                        // mapやからリストの内容一つ一つ見ていくんか
                        // で、イベントからもらってない奴はそれをそのまま返すんか
    ...
    private fun trackFood(event: SearchEvent.OnTrackFoodClick) {
        viewModelScope.launch {
            ...
            // Food, Amountは、各イベントでStateで管理しているからそれを引っ張ってくる
